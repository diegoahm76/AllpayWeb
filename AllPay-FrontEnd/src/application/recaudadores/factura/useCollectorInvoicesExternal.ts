...
import { getFacturasExternas } from '@/adapters/recaudadores/facturas/collector.invoices.external';
...
const useCollectorInvoices = () => {
    ...
    const fetchInvoices = async (token: string, collectorIds: number[], page: number = 1, searchParams?: SearchParams) => {
        ...
        try {
            const response = await getFacturasExternas(token, collectorIds, page, searchParams);
...
export default useCollectorInvoices;
