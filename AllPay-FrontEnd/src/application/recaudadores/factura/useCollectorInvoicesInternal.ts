...
import {
  getCollectorInvoices,
  CollectorInvoice,
  CollectorInvoicesResponse,
  GetCollectorInvoicesParams
} from '@/adapters/recaudadores/facturas/collector.invoices.internal';
...
const useCollectorInvoicesInternal = (): UseCollectorInvoicesReturn => {
  ...
  const fetchInvoices = useCallback(
    async (token: string, params?: GetCollectorInvoicesParams): Promise<CollectorInvoicesResponse> => {
      try {
        ...
        const response = await getCollectorInvoices(cleanedParams, token);
...
export default useCollectorInvoicesInternal;
