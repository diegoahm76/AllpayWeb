...
const ComprobanteComponent: React.FC = () => {
  ...
  // Efecto para obtener el código de pago realizado
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // Primero intentar obtener desde localStorage (método preferido)
      const openPayId = localStorage.getItem('pse_openpay_id');
      
      if (openPayId) {
        setCodPagoRealizado(openPayId);
        // Limpiar localStorage después de obtener el ID
        localStorage.removeItem('pse_openpay_id');
      } else if (reference) {
        // Fallback: usar parámetros de URL
        setCodPagoRealizado(reference);
      } else {
        console.warn('[ComprobanteComponent] - No se encontró código de pago en localStorage ni en URL');
      }
    }
  }, [reference]);
      ...
      <div className="flex justify-center items-center h-[500px]">
        <div className={`p-6 rounded-lg ${theme === 'dark' ? 'bg-yellow-900 text-yellow-100' : 'bg-yellow-50 text-yellow-700'}`}>
          <div className="text-center">
            <p className="mb-4">No se encontró la referencia del pago.</p>
            <p className="text-sm">
              Esto puede ocurrir si accedió directamente a esta página. 
