...
import { useAccionesCobroSimple } from '../hooks/useAccionesCobroSimple';
// Importar el adaptador de registro de acciones
import { createCobroPersuasivo } from '../adapters/registroAccion.adapter';
import { CreateCobroPersuasivoPayload } from '../models/registroAccion.model';
...
const RegistroAccionesCobroPesuasivo: React.FC<RegistroAccionesCobroPesuasivoProps> = ({ 
  ...
  const handleConfirmSubmit = useCallback(async () => {
    ...
    try {
      ...
      const payload: CreateCobroPersuasivoPayload = {
        factura: parseInt(facturaId),
        descripcion: formData.descripcionAccion,
        fecha_registro: new Date(formData.fechaRegistro + 'T' + new Date().toTimeString().split(' ')[0] + 'Z').toISOString(),
      ...
      const response = await createCobroPersuasivo(token, payload);
            
      setAlertMessage(`Acción de cobro registrada exitosamente. Consecutivo: ${response.data.consecutivo}`);
      ...
      <div className={`rounded-xl p-6 ${isDarkMode ? 'bg-opacity-10 bg-[#78390e]' : 'bg-slate-200'}`}>
        <div className={`rounded-xl p-6 relative ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'}`}>
          ...
          <h3 className={`text-2xl font-bold text-center mb-6 ${isDarkMode ? 'text-white' : 'text-[#562707]'}`}>
            {isEditMode ? 'Editar Acción Cobro Persuasivo' : 'Registrar Acciones Cobro Persuasivo'}
          </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <SecureInput
                  ...
                  value={facturaDetalles ? formatDateForDisplay(facturaDetalles.recaudador_info.fecha_compra) : formatDateForDisplay(formData.fechaCompra)}
                  type="text"
                  darkMode={isDarkMode}
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                {errors.descripcionAccion && (
                  <p className={`text-xs mt-1 ${isDarkMode ? 'text-red-300' : 'text-red-500'}`}>La descripción de la acción es obligatoria</p>
                )}
              </div>
              <div className="grid grid-cols-1 gap-4">
                <AnimatedSelect
