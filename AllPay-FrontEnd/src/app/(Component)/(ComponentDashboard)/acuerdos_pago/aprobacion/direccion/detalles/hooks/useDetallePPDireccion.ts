...
import { DetallePlanPago } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/direccion/detalles/models/DetallePPDireccion.model';
import { detallePlanPagoDireccionAdapter } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/direccion/detalles/adapters/detallePPDireccion.adapter';

export const useDetallePlanPagoDireccion = (id: string, token: string) => {
    ...
    const fetchDetallePlanPago = async () => {
        try {
            ...
            const result = await detallePlanPagoDireccionAdapter(id, token);
            setData(result);
        } catch (err) {
