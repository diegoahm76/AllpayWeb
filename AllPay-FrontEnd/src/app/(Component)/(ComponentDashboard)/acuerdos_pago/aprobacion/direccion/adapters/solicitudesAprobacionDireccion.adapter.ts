...
import { SolicitudesAprobacionResponse } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/direccion/models/solicitudesAprobacion.model';

const baseApiUrl = process.env.BASE_API_URL;

export interface SolicitudesAprobacionDireccionFilters {
    page?: number;
    page_size?: number;
    estado?: string;
...
export const getSolicitudesAprobacionDireccion = async (
    token: string,
    filters: SolicitudesAprobacionDireccionFilters = {}
    ...
    try {
        ...
        const response = await axios.get<SolicitudesAprobacionResponse>(
            `${baseApiUrl}recaudos/acuerdos-pago/aprobacion/direccion/obtener-solcitiudes-aprobacion/?${params.toString()}`,
            {
                headers: {
