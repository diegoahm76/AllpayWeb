import { DetallePlanPago, DetallePlanPagoResponse } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/juridica/detalles/models/DetallePPJuridica.model';

export const detallePlanPagoAdapter = async (id: string, token: string): Promise<DetallePlanPago[]> => {
    try {
        const baseApiUrl = process.env.BASE_API_URL;
        const response = await fetch(
            `${baseApiUrl}recaudos/acuerdos-pago/aprobacion/juridica/detalles-planes-acuerdo-pago/${id}/`,
            {
                method: 'GET',
                headers: {
