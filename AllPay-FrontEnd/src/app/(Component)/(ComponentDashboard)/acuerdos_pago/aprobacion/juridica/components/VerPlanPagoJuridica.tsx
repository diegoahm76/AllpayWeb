...
// components
import AprobarPlanPagoJuridico from "@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/juridica/components/AprobarPlanPagoJuridico";

// hooks
import { useSolicitudesAprobacion } from "@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/juridica/hooks/useObtenerPPJuridico";
import { useGenerarYDescargarDocumento } from "@/application/documento/useGenerar&DescargarDocumento";
import { useConsultarDatosDocumentoPlanesPago } from "@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/juridica/hooks/useConsultarDatosDocumentoPlanesPagoAprobacionJuridica";

export default function VerPlanPagoJuridico() {
    ...
    const {
        ...
        fetchAllData,
        fetchData
    } = useSolicitudesAprobacion(token, 1, 10, true); // shouldFetch = true para hacer petición automática
    ...
    const handleOpenModal = (idPlanPago: string, idSolicitud: string) => {
                ...
                <div className="w-full max-w-full mx-auto ">
                    <div className={`rounded-xl p-6 ${isDarkMode ? 'bg-opacity-10 bg-[#78390e]' : 'bg-slate-200'}`}>
                        ...
                        <div className={`rounded-xl p-6 relative ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'}`}>
                            ...
                            <h1 className={`text-xl sm:text-2xl lg:text-3xl mt-6 font-bold text-center ${isDarkMode ? 'text-white' : 'text-[rgb(var(--brown))]'}`}>APROBACIÓN DE ACUERDOS DE PAGO - ÁREA DE RECAUDO JURÍDICO</h1>
                                    ...
                                    actions={[
                                        {
                                            ...
                                            render: (row) => (
                                                <div className="flex gap-2">
                                                    <IconButton
                                                        onClick={() => {
                                                            router.push(`/acuerdos_pago/aprobacion/juridica/detalles?id=${row.id_solicitud}`);
                                                        }}
                                                        sx={{
