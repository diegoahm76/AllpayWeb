...
import { aprobacionPlanPagoJuridicaAdapter } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/aprobacion/juridica/adapters/aprobacionPlanPagoJuridica.adapter';
...
export const useAprobacionPlanPago = () => {
    ...
    const aprobarPlanPago = async (id: string, token: string, requestData: AprobacionPlanPagoRequest) => {
        try {
            ...
            const response = await aprobacionPlanPagoJuridicaAdapter(id, token, requestData);
            setData(response);
            return response;
        } catch (err) {
