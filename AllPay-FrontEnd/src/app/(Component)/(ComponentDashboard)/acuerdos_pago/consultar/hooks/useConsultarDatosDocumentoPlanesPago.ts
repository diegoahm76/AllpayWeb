...
import { getConsultaPlanesPagoExterno } from '@/app/(Component)/(ComponentDashboard)/acuerdos_pago/consultar/adapters/consultaDatosDocumentoPlanesPagoExterno.adapter';
  ...
  const fetchDatosDocumento = useCallback(async (id: number | string) => {
    ...
    try {
      const response = isInternalUser 
        ? await getConsultaPlanesPagoInterno(token, id) 
        : await getConsultaPlanesPagoExterno(token, id);
      setData(response);
      return response;
    } catch (err) {
