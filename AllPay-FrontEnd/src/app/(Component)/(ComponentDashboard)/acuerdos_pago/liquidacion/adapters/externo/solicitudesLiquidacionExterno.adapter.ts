...
export const getSolicitudesLiquidacionExterno = async (
    token: string,
    ...
    try {
        ...
        const response = await axios.get<SolicitudesLiquidacionResponse>(
            `${baseApiUrl}recaudos/acuerdos-pago/liquidacion-externo/obtener-planes-acuerdo-pago/?${params.toString()}`,
            {
                headers: {
        ...
        if (axios.isAxiosError(error)) {
            throw new Error(error.response?.data?.detail || 'Error al consultar solicitudes de liquidación');
        }
        throw new Error('Error al consultar solicitudes de liquidación');
    }
}; 