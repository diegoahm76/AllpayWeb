...
const TablaAccionesCobro: React.FC<TablaAccionesCobroProps> = ({ 
  ...
  // Función para renderizar el botón de documento (actualizada para nueva estructura)
  const renderDocumento = (_value: string, row: CobroPersuasivoTabla) => {
    // Usar los nuevos campos de la estructura actualizada
    const tieneDocumento = row.tieneDocumento && row.documentoAdjuntoUrl;
    ...
    return (
      <button
        ...
        className="flex items-center space-x-2 p-2 rounded hover:bg-green-100 dark:hover:bg-green-900 transition-colors text-sm"
        title={`Descargar: ${row.nombreArchivo || 'documento'}`}
      >
        <FileDownload 
          sx={{ 
            fontSize: 20,
  ...
  const handleVerDocumento = (registro: CobroPersuasivoTabla) => {
    console.log('[TablaAccionesCobro] - Intentando abrir documento:', {
      tieneDocumento: registro.tieneDocumento,
      nombreArchivo: registro.nombreArchivo,
      url: registro.documentoAdjuntoUrl
    });
    
    // Si existe la URL del documento adjunto, abrirlo directamente
    if (registro.tieneDocumento && registro.documentoAdjuntoUrl) {
      ...
      console.log('[TablaAccionesCobro] - No hay URL directa, intentando descarga desde API');
  ...
  const handleRegistrarAccion = () => {
    ...
    const coactivoId = searchParams.get('coactivoId') || '';
    const params = new URLSearchParams({
      facturaId: facturaId,
      nroFactura: nroFactura
    });
    if (coactivoId) params.append('coactivoId', coactivoId);

    router.push(`/coactivo/formulario?${params.toString()}`);
  };
        ...
        <div className={`rounded-xl p-6 relative ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'}`}>
          ...
          <div className="mb-6">
            <h3 className={`text-2xl font-bold text-center ${isDarkMode ? 'text-white' : 'text-[#562707]'}`}>
              Registro Acciones Cobro Coactivo
            </h3>
          </div>
