...
const transformApiDataToCarteraMora = (apiData: FacturaCarteraMoraAPI): FacturaCarteraMora => {
  ...
  return {
    // usar id único por fila si viene; si no, fallback al id de factura
    id: apiData.id_row_numeric ?? apiData.id_factura_unica,
    id_factura_unica: apiData.id_factura_unica,
    id_cobro_coactivo: (apiData as any).id_cobro_coactivo,
...
export const useCarteraMora = () => {
  ...
  // Función para formatear fechas para Excel (YYYY-MM-DD)
  const formatDateForExcel = useCallback((dateString: string) => {
  ...
  const formatDataForExcel = useCallback((data: FacturaCarteraMora[]) => {
    return data.map(row => ({
      ...row,
      ...
      valor_intereses: Number(row.valor_intereses ?? 0),
      // Mantener otros campos tal cual
      documento_recaudador: row.documento_recaudador,
      razon_social_recaudador: row.razon_social_recaudador,
      estado_acuerdo_pago: row.estado_acuerdo_pago
    }));
  }, [formatDateForExcel]);
  ...
  const fetchAllDataForExcel = useCallback(async () => {
    ...
    try {
      ...
      return {
        data: formatDataForExcel(carteraTransformada),
        total_pages: 1
      };
  ...
  }, [formData, token, formatDataForExcel]);
