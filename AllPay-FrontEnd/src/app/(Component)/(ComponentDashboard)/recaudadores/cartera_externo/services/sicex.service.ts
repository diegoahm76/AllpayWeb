...
const obtenerTotalesCartera = async ({
  ...
  try {
    ...
    const url = `${baseApiUrl}cartera/cartera-consulta-totales/${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
...
const buscarResumenSicex = async ({
  ...
  try {
    ...
    const url = `${baseApiUrl}cartera/cartera-consulta-externo/?${queryParams.toString()}`;
    ...
    // Cargar totales en paralelo (sin bloquear)
    ...
    if (isNoRecordsFound) {
      ...
      // Limpiar los totales cuando no hay registros
      setResumen_totales({
        total_kilos: 0,
...
// Nueva función para cargar totales de forma asíncrona e independiente
const cargarTotalesEnParalelo = async ({
  ...
  try {
    console.log('🔄 Cargando totales externo en paralelo...');
    
    const totalesResponse = await obtenerTotalesCartera({ valueSesion, filters });
    
    if (totalesResponse.success && totalesResponse.data) {
      console.log('✅ Totales externo cargados exitosamente:', totalesResponse.data);
      setResumen_totales(totalesResponse.data);
    } else {
      console.warn('⚠️ No se pudieron obtener los totales externo:', totalesResponse.error);
      // Mantener valores por defecto si ya estaban establecidos
    ...
    console.error('❌ Error al cargar totales externo en paralelo:', error);
...
// Nueva función: obtener todos los datos sin paginación para exportar a Excel
const fetchAllResumenSicexExcelSinPaginacion = async ({
  ...
  // El endpoint con sin_paginacion=true retorna los datos en la raíz (data),
  // pero mantenemos compatibilidad si viniera anidado en results.
  ...
  // Convertir campos numéricos a Number para que Excel pueda operar con ellos
  const numericKeys = [
    'total_kilos',
    'valor_bruto',
    'cuota_fomento',
...
// Función para obtener y abrir directamente el archivo PDF
const openPdfDirectly = async (fileUrl: string, token: string): Promise<void> => {
  try {
    ...
    try {
      ...
      console.log('Error al intentar con token, probando sin token');
      // Segundo intento: sin token
      response = await axios.get(fileUrl);
    }
    ...
    if (response.data && response.data.success && response.data.archivo) {
      console.log('URL del archivo encontrada:', response.data.archivo);
