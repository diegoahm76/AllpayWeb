...
const Administracion_cargos: React.FC = () => {
  ...
  const handleEdit = (row: any) => {
    setEditId(row.consecutivo_sicex);

    // Parsear string tipo "14/4/2025, 8:26:25 p. m." → formato dd/MM/yyyy
    let fechaCargueFormateada = '';
    if (row?.fecha_cargue) {
      const partesFecha = row.fecha_cargue.split(',')[0].trim(); // "14/4/2025"
      const [dia, mes, anio] = partesFecha.split('/');
      fechaCargueFormateada = `${dia.padStart(2, '0')}/${mes.padStart(2, '0')}/${anio}`;
    }
    ...
    setFormData({
      ...
      tipo_cargue: row?.tipo_cargue || '',
      archivo: null,
      fecha_cargue: fechaCargueFormateada
    });

    setCantidadRegistros(row?.cantidad_registros ?? '');
    setConfigurar(false);
  ...
  const [totalPagesDetalle, setTotalPagesDetalle] = useState(1);
  ...
  const fetchAllDetalleData = async (page: number) => {
    ...
    try {
      ...
      // La respuesta tiene estructura anidada: response.data.data.data contiene el array
      const lista = response.data.data?.data || [];
      const totalPages = response.data.total_pages || 1;
      
      // Actualizar el estado con los nuevos datos
      setDetalleData(lista);
      setTotalPagesDetalle(totalPages);
  ...
  const handlePageChangeDetalle = async (page: number) => {
    setCurrentPageDetalle(page);
    await fetchAllDetalleData(page);
  };

  const fetchDataForExcel = async (): Promise<{ data: any[]; total_pages: number }> => {
    if (!idSeleccionadoDetalle || !valueSesion) {
      console.error('❌ [EXCEL] Faltan parámetros: idSeleccionadoDetalle o valueSesion');
      return { data: [], total_pages: 1 };
    }

    try {
      console.log('🔄 [EXCEL] Iniciando petición para Excel con sin_paginacion=true');
      const response = await axios.get(
        `${baseApiUrl}cartera/sicex-cargue/list/${idSeleccionadoDetalle}/?sin_paginacion=true`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${valueSesion.user.tokens.access}`
      ...
      console.log('✅ [EXCEL] Respuesta recibida:', {
        success: response.data.success,
        hasData: !!response.data.data,
        dataType: typeof response.data.data,
        isArray: Array.isArray(response.data.data),
        dataStructure: response.data.data ? Object.keys(response.data.data) : 'no data'
      });

      if (!response.data.success) {
        console.error('❌ [EXCEL] La respuesta no es exitosa:', response.data.detail);
      ...
      // Caso 1: Estructura anidada con paginación (response.data.data.data)
      if (response.data.data?.data && Array.isArray(response.data.data.data)) {
        lista = response.data.data.data;
        console.log('📊 [EXCEL] Estructura detectada: response.data.data.data');
      }
      // Caso 2: Estructura directa (response.data.data es array)
      else if (response.data.data && Array.isArray(response.data.data)) {
        lista = response.data.data;
        console.log('📊 [EXCEL] Estructura detectada: response.data.data (array directo)');
      }
      // Caso 3: Si data es un objeto con una propiedad que contiene el array
      else if (response.data.data && typeof response.data.data === 'object' && !Array.isArray(response.data.data)) {
        // Buscar cualquier propiedad que sea un array
        const keys = Object.keys(response.data.data);
        for (const key of keys) {
          if (Array.isArray(response.data.data[key])) {
            lista = response.data.data[key];
            console.log(`📊 [EXCEL] Estructura detectada: response.data.data.${key}`);
            break;
          }
        }
      }
      // Caso 4: La respuesta completa es un array (poco probable pero posible)
      else if (Array.isArray(response.data)) {
        lista = response.data;
        console.log('📊 [EXCEL] Estructura detectada: response.data (array directo)');
      }

      console.log('📊 [EXCEL] Total de registros obtenidos:', lista.length);
      
      if (lista.length === 0) {
        console.warn('⚠️ [EXCEL] No se encontraron datos. Estructura completa de respuesta:', JSON.stringify(response.data, null, 2));
      }
      ...
      console.error('❌ [EXCEL] Error al obtener datos para Excel:', error);
      console.error('❌ [EXCEL] Detalles del error:', {
  ...
  const handleVerDetalle = async (id: number, tipoCargue: string) => {
    try {
      ...
      const response = await axios.get(`${baseApiUrl}cartera/sicex-cargue/list/${id}/?page=1`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${valueSesion.user.tokens.access}`
        }
    ...
    <div className="w-full">
      <ModalContainer
        ...
        onClose={() => {
          ...
          setDetalleData([]);
          setTotalPagesDetalle(1);
          setCurrentPageDetalle(1);
          setIdSeleccionadoDetalle(null);
        }}
        size="full"
      >
        <h3
        ...
        >
          Consulta Datos de Cargue - {tipoCargueSeleccionado ? tipoCargueSeleccionado.toUpperCase() : 'EXPORTACIÓN'}
        </h3>
        {(() => {
          ...
          return (
            <DynamicTable
              ...
              onPageChange={handlePageChangeDetalle}
                  ...
                  <Grid container spacing={2}>
                    ...
                    >
                      ...
                      <AlertQuestion
                        ...
                        onConfirm={() => {
                          // Cerrar el AlertQuestion inmediatamente
                          setAlertOpen(false);
                          // Activar el loader antes de ejecutar la creación
                      ...
                      <AlertLoader
                        isOpen={isLoadingCrear}
                        loadingText="Creando registro, por favor espere…"
                      />
                    </Grid>
                  </Grid>
                </div>
