...
export const useLiquidacionesPagadas = (token?: string) => {
    ...
    const fetchLiquidaciones = useCallback(async (pageOrFilters: number | LiquidacionesPagadasFilters = 1) => {
        ...
        try {
            ...
            console.log('[useLiquidacionesPagadas] - Obteniendo liquidaciones:', { pageOrFilters });
            
            const response = await getLiquidacionesPagadas(token, pageOrFilters);
    ...
    const obtenerLiquidaciones = useCallback((filters?: LiquidacionesPagadasFilters) => {
        setCurrentPage(1);
        if (filters) {
            fetchLiquidaciones({ ...filters, page: 1 });
        } else {
            fetchLiquidaciones(1);
        }
    }, [fetchLiquidaciones]);
    ...
    const cambiarPagina = useCallback(async (nuevaPagina: number) => {
        if (nuevaPagina >= 1 && nuevaPagina <= totalPages) {
            await fetchLiquidaciones(nuevaPagina);
        }
