...
const baseApiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://allpayfedecacao.com/apii/';

const Administracion_cargos: React.FC = (): React.ReactElement => {
  ...
  // Nueva función para Excel interno: obtener todos los datos en una sola llamada
      ...
      render: (row: any) => (
        <IconButton
          ...
          onClick={async () => {
            ...
            try { 
              const downloadUrl = `${baseApiUrl}cartera/cartera-consulta-download/${row.id_factura_unica}/1/`;
      ...
      <div className="flex w-full items-center justify-center md:w-4/4 lg:w-2/2">
            ...
            className={`m-auto w-full rounded-xl p-6 ${isDarkMode ? 'bg-opacity-10 bg-[#78390e]' : 'bg-slate-200'}`}
              ...
              className={`rounded-xl p-4 relative ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'}`}
                  ...
                  <Grid container spacing={2} marginTop={2}>
                    ...
                    <Grid item xs={12} sm={4}>
                      <AnimatedInput
                        ...
                        onChange={(e) => handleFilterChange('nro_factura', e.target.value)}
                        type="number"
                        darkMode={isDarkMode}
                      />
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <AnimatedSelect
                        name="tiene_acuerdo_pago"
                        label="Tiene Acuerdo de Pago"
                        value={filters?.tiene_acuerdo_pago}
                        onChange={(e) => handleFilterChange('tiene_acuerdo_pago', e.target.value)}
                        options={[
                ...
                <div
                  className={`mt-4 rounded-xl p-4 ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'} relative`}
                >
                  <DynamicTable
                    columns={columns}
                    data={formattedData}
