...
import { descargarDocumentoPagosLinea } from '../adapters/descargarDocumentoPagosLinea.adapter';
import { 
    DescargarDocumentoPagosLineaMapped,
...
const INITIAL_DATA: DescargarDocumentoPagosLineaMapped = {
    success: false,
    detail: '',
...
/**
 * Hook personalizado para manejar la generación del documento del reporte de pagos en línea
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar la generación del documento
 */
export const useDescargarDocumentoPagosLinea = (token: string) => {
    // Estados principales
    const [descargaData, setDescargaData] = useState<DescargarDocumentoPagosLineaMapped>(INITIAL_DATA);
    ...
    const descargarDocumento = useCallback(async (params?: DescargarDocumentoPagosLineaParams) => {
        if (!token) {
        ...
        try {
            ...
            console.log('[useDescargarDocumentoPagosLinea] - Iniciando generación de documento con parámetros:', params);

            const response = await descargarDocumentoPagosLinea(token, params || {});

            // Mapear la respuesta al formato que usaremos
            const mappedResponse: DescargarDocumentoPagosLineaMapped = {
            ...
            if (mappedResponse.success && mappedResponse.archivo) {
                console.log('[useDescargarDocumentoPagosLinea] - Abriendo documento en nueva pestaña:', mappedResponse.archivo);
            ...
            console.error('[useDescargarDocumentoPagosLinea] - Error:', err);
