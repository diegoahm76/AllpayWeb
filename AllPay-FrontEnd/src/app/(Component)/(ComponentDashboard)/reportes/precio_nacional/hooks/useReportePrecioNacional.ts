...
import { getReportePrecioNacional } from '../adapters/reportePrecioNacional.adapter';
import { 
...
const INITIAL_PARAMS: ReportePrecioNacionalParams = {
    page: 1,
    page_size: 10
};

// Estado inicial para los datos del reporte
const INITIAL_DATA: ReportePrecioNacionalMapped = {
    success: false,
    recaudadores: [],
    totales: {
        total_kilos: 0,
...
/**
 * Hook personalizado para manejar el reporte de precio nacional de cacao
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar el reporte
 */
export const useReportePrecioNacional = (token: string) => {
    // Estados principales
    const [reporteData, setReporteData] = useState<ReportePrecioNacionalMapped>(INITIAL_DATA);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    
    // Estados para parámetros de consulta
    const [params, setParams] = useState<ReportePrecioNacionalParams>(INITIAL_PARAMS);
    ...
    const updateParams = useCallback((newParams: Partial<ReportePrecioNacionalParams>) => {
        setParams(prev => ({
            ...prev,
            ...newParams,
    ...
    const fetchReporte = useCallback(async (customParams?: Partial<ReportePrecioNacionalParams>) => {
        if (!token) {
        ...
        try {
            ...
            const response = await getReportePrecioNacional(token, requestParams);

            setReporteData(response);
            
            if (response.recaudadores.length === 0) {
            ...
            console.error('[useReportePrecioNacional] - Error:', err);
    ...
    const fetchAllData = useCallback(async (page: number) => {
        if (!token) {
            console.error('[useReportePrecioNacional] - No hay token de autenticación disponible');
        ...
        try {
            ...
            console.log('[useReportePrecioNacional] - Obteniendo datos para Excel con parámetros:', requestParams);

            const response = await getReportePrecioNacional(token, requestParams);
            
            return {
                data: response.recaudadores,
                total_pages: response.total_pages
            };
        } catch (error) {
            console.error('[useReportePrecioNacional] - Error en fetchAllData:', error);
            return {
                data: [],
                total_pages: 0
            };
