...
import { getReporteLibroComprasCacao } from '../adapters/reporteLibroComprasCacao.adapter';
import { 
...
const INITIAL_PARAMS: ReporteLibroComprasCacaoParams = {
    page: 1,
    page_size: 10
};

// Estado inicial para los datos del reporte
const INITIAL_DATA: ReporteLibroComprasCacaoMapped = {
    success: false,
    facturas: [],
    totales: {
...
/**
 * Hook personalizado para manejar el reporte consolidado de libro de compras de cacao
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar el reporte
 */
export const useReporteLibroComprasCacao = (token: string) => {
    // Estados principales
    const [reporteData, setReporteData] = useState<ReporteLibroComprasCacaoMapped>(INITIAL_DATA);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    
    // Estados para parámetros de consulta
    const [params, setParams] = useState<ReporteLibroComprasCacaoParams>(INITIAL_PARAMS);
    ...
    const updateParams = useCallback((newParams: Partial<ReporteLibroComprasCacaoParams>) => {
        setParams(prev => ({
            ...prev,
            ...newParams,
    ...
    const fetchReporte = useCallback(async (customParams?: Partial<ReporteLibroComprasCacaoParams>) => {
        if (!token) {
        ...
        try {
            ...
            console.log('[useReporteLibroComprasCacao] - Obteniendo reporte con parámetros:', requestParams);

            const response = await getReporteLibroComprasCacao(token, requestParams);
            ...
            console.error('[useReporteLibroComprasCacao] - Error:', err);
    ...
    const handlePageChange = useCallback((newPage: number) => {
        ...
        const fetchWithNewPage = async () => {
            try {
                ...
                console.log('[useReporteLibroComprasCacao] - Cambiando a página:', newPage);
                console.log('[useReporteLibroComprasCacao] - Parámetros completos:', newParams);

                const response = await getReporteLibroComprasCacao(token, newParams);
                setReporteData(response);
                ...
                const errorMessage = err instanceof Error ? err.message : 'Error inesperado al obtener el reporte';
                console.error('[useReporteLibroComprasCacao] - Error al cambiar página:', err);
    ...
    const applyFilters = useCallback((filters: Partial<ReporteLibroComprasCacaoParams>) => {
    ...
    const fetchAllData = useCallback(async (page: number) => {
        if (!token) {
            console.error('[useReporteLibroComprasCacao] - No hay token de autenticación disponible');
        ...
        try {
            ...
            console.log('[useReporteLibroComprasCacao] - Obteniendo datos para Excel con parámetros:', requestParams);
            ...
            console.error('[useReporteLibroComprasCacao] - Error en fetchAllData:', error);
            return {
                data: [],
                total_pages: 0
            };
