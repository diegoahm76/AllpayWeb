...
import { getReporteAcuerdosPago } from '@/app/(Component)/(ComponentDashboard)/reportes/cuota_fomento_fecha/adapters/reporteAcuerdosPago.adapter';
import { 
...
/**
 * Hook personalizado para manejar el reporte de acuerdos de pago
...
export const useReporteAcuerdosPago = (token: string) => {
    ...
    const fetchAllData = useCallback(async (page: number) => {
        ...
        try {
            ...
            console.log('[useReporteAcuerdosPago] - Obteniendo datos para Excel con parámetros:', requestParams);
