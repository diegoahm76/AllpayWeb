...
import { getReporteInteresesAcuerdoPago } from '@/app/(Component)/(ComponentDashboard)/reportes/cuota_fomento_fecha/adapters/reporteInteresesAcuerdoPago.adapter';
import { 
...
const INITIAL_PARAMS: ReporteInteresesAcuerdoPagoParams = {
    page: 1,
    page_size: 10
};

// Estado inicial para los datos del reporte
const INITIAL_DATA: ReporteInteresesAcuerdoPagoMapped = {
    success: false,
    registros: [],
    sumatorias: {
...
/**
 * Hook personalizado para manejar el reporte de intereses de acuerdo de pago
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar el reporte
 */
export const useReporteInteresesAcuerdoPago = (token: string) => {
    // Estados principales
    const [reporteData, setReporteData] = useState<ReporteInteresesAcuerdoPagoMapped>(INITIAL_DATA);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    
    // Estados para parámetros de consulta
    const [params, setParams] = useState<ReporteInteresesAcuerdoPagoParams>(INITIAL_PARAMS);
    ...
    const updateParams = useCallback((newParams: Partial<ReporteInteresesAcuerdoPagoParams>) => {
        setParams(prev => ({
            ...prev,
            ...newParams,
    ...
    const fetchReporte = useCallback(async (customParams?: Partial<ReporteInteresesAcuerdoPagoParams>) => {
        if (!token) {
        ...
        try {
            ...
            const response = await getReporteInteresesAcuerdoPago(token, requestParams);
            ...
            console.error('[useReporteInteresesAcuerdoPago] - Error:', err);
    ...
    const fetchAllData = useCallback(async (page: number) => {
        if (!token) {
            console.error('[useReporteInteresesAcuerdoPago] - No hay token de autenticación disponible');
        ...
        try {
            ...
            console.log('[useReporteInteresesAcuerdoPago] - Obteniendo datos para Excel con parámetros:', requestParams);
            ...
            console.error('[useReporteInteresesAcuerdoPago] - Error en fetchAllData:', error);
            return {
                data: [],
                total_pages: 0
            };
