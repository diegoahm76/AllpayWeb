...
import { descargarDocumentoInteresesAcuerdoPago } from '../adapters/descargarDocumentoInteresesAcuerdoPago.adapter';
import { 
    DescargarDocumentoInteresesAcuerdoPagoMapped,
...
const INITIAL_DATA: DescargarDocumentoInteresesAcuerdoPagoMapped = {
    success: false,
    detail: '',
...
/**
 * Hook personalizado para manejar la generación del documento del reporte de intereses de acuerdo de pago
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar la generación del documento
 */
export const useDescargarDocumentoInteresesAcuerdoPago = (token: string) => {
    // Estados principales
    const [descargaData, setDescargaData] = useState<DescargarDocumentoInteresesAcuerdoPagoMapped>(INITIAL_DATA);
    ...
    const descargarReporte = useCallback(async (params?: DescargarDocumentoInteresesAcuerdoPagoParams) => {
        if (!token) {
        ...
        try {
            ...
            console.log('[useDescargarDocumentoInteresesAcuerdoPago] - Iniciando generación de documento con parámetros:', params);

            const response = await descargarDocumentoInteresesAcuerdoPago(token, params || {});

            // Mapear la respuesta al formato que usaremos
            const mappedResponse: DescargarDocumentoInteresesAcuerdoPagoMapped = {
            ...
            if (mappedResponse.success && mappedResponse.archivo) {
                console.log('[useDescargarDocumentoInteresesAcuerdoPago] - Abriendo documento en nueva pestaña:', mappedResponse.archivo);
            ...
            console.error('[useDescargarDocumentoInteresesAcuerdoPago] - Error:', err);
