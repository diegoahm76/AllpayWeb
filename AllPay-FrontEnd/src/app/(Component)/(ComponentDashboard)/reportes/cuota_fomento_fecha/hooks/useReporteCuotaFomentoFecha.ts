...
import { getReporteCuotaFomentoFecha } from '@/app/(Component)/(ComponentDashboard)/reportes/cuota_fomento_fecha/adapters/reporteCuotaFomentoFecha.adapter';
import { 
...
const INITIAL_PARAMS: ReporteCuotaFomentoFechaParams = {
    page: 1,
    page_size: 10
};

// Estado inicial para los datos del reporte
const INITIAL_DATA: ReporteCuotaFomentoFechaMapped = {
    success: false,
    registros: [],
    sumatorias: {
        sumatoria_kilos: 0,
...
/**
 * Hook personalizado para manejar el reporte de cuota fomento por fecha
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar el reporte
 */
export const useReporteCuotaFomentoFecha = (token: string) => {
    // Estados principales
    const [reporteDataCFR, setReporteDataCFR] = useState<ReporteCuotaFomentoFechaMapped>(INITIAL_DATA);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    
    // Estados para parámetros de consulta
    const [params, setParams] = useState<ReporteCuotaFomentoFechaParams>(INITIAL_PARAMS);
    ...
    const updateParams = useCallback((newParams: Partial<ReporteCuotaFomentoFechaParams>) => {
        setParams(prev => ({
            ...prev,
            ...newParams,
    ...
    const fetchReporte = useCallback(async (customParams?: Partial<ReporteCuotaFomentoFechaParams>) => {
        if (!token) {
        ...
        try {
            ...
            const response = await getReporteCuotaFomentoFecha(token, requestParams);

            setReporteDataCFR(response);
            ...
            console.error('[useReporteCuotaFomentoFecha] - Error:', err);
            
            setError(errorMessage);
            showErrorAlertMessage(errorMessage);
            
            // Limpiar datos en caso de error
            setReporteDataCFR(INITIAL_DATA);
        } finally {
            setIsLoading(false);
        }
