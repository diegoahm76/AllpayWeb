...
import { descargarDocumentoAcuerdosPago } from '../adapters/descargarDocumentoAcuerdosPago.adapter';
import { 
    DescargarDocumentoAcuerdosPagoMapped,
...
const INITIAL_DATA: DescargarDocumentoAcuerdosPagoMapped = {
    success: false,
    detail: '',
...
/**
 * Hook personalizado para manejar la generación del documento del reporte de acuerdos de pago
 * @param token Token de autenticación JWT
 * @returns Objeto con estados y funciones para manejar la generación del documento
 */
export const useDescargarDocumentoAcuerdosPago = (token: string) => {
    // Estados principales
    const [descargaData, setDescargaData] = useState<DescargarDocumentoAcuerdosPagoMapped>(INITIAL_DATA);
    ...
    const descargarReporte = useCallback(async (params?: DescargarDocumentoAcuerdosPagoParams) => {
        if (!token) {
        ...
        try {
            ...
            console.log('[useDescargarDocumentoAcuerdosPago] - Iniciando generación de documento con parámetros:', params);

            const response = await descargarDocumentoAcuerdosPago(token, params || {});

            // Mapear la respuesta al formato que usaremos
            const mappedResponse: DescargarDocumentoAcuerdosPagoMapped = {
            ...
            if (mappedResponse.success && mappedResponse.archivo) {
                console.log('[useDescargarDocumentoAcuerdosPago] - Abriendo documento en nueva pestaña:', mappedResponse.archivo);
            ...
            console.error('[useDescargarDocumentoAcuerdosPago] - Error:', err);
