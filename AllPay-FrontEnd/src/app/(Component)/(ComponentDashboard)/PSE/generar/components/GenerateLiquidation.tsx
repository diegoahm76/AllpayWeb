...
const SearchLiquidation = () => {
    ...
    const actions = [
        {
            ...
            render: (_: any) => (
                <IconButton
                    onClick={() => handlePagarPSE(_.id_factura_unica, _.id_liq_factura_unica)}
                    sx={{
    ...
    const fetchDataForExcel = async (): Promise<{ data: any[]; total_pages: number; }> => {
        if (isInternalUser === null || !valueSesion?.user?.tokens?.access) {
            return { data: [], total_pages: 1 };
        }

        try {
            if (isInternalUser) {
                // Para usuarios internos - llamada directa al adapter
                const params: any = {
                    sin_paginacion: true
                };
                if (formData.tipoDocumento) params.tipo_documento_proveedor = formData.tipoDocumento;
                ...
                // Importación dinámica para evitar actualizar el estado del hook
                const { getCollectorInvoices } = await import('@/adapters/recaudadores/facturas/collector.invoices.internal');
                const response = await getCollectorInvoices(params, valueSesion.user.tokens.access);
                return {
                    ...response,
                    data: formatDataForExcel(response.data)
                };
            } else {
                // Para usuarios externos (PSE) - llamada directa al adapter
                const params: any = {
                    sin_paginacion: true
                };
                if (formData.nroFacturaUnica) params.nro_factura_unica = formData.nroFacturaUnica;
                ...
                // Importación dinámica para evitar actualizar el estado del hook
                const { getFacturasLiquidadasPendientesPSE } = await import('../adapters/getFacturasLiquidadasPendientesPSE');
                const response = await getFacturasLiquidadasPendientesPSE(valueSesion.user.tokens.access, 1, params);
                return {
                    ...response,
                    data: formatDataForExcel(response.data),
    ...
    const handlePagarPSE = async (idFactura: number, idLiqFacturaUnica?: number | null) => {
        try {
            setIsAlertLoader(true);
            ...
            if (facturaDetalle && facturaDetalle.success) {
                ...
                sessionStorage.setItem('pse_payment_data', JSON.stringify({
                    ...
                    id_liq_factura_unica: idLiqFacturaUnica,
    ...
    return (
        <div className="space-y-6">
            ...
            >
                ...
                <div className={`rounded-3xl shadow-md p-6 relative ${isDarkMode ? 'bg-[#260f00] text-white border border-white/20' : 'bg-white'}`}>
                <button
                        onClick={() => router.push('/')}
                    ...
                    <h2 className={` text-xl sm:text-2xl lg:text-3xl font-bold text-center my-6 ${isDarkMode ? 'text-white' : 'text-[#562707]' }`} >
