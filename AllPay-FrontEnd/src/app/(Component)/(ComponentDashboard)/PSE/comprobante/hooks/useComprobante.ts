...
export const useComprobante = ({ codPagoRealizado, autoFetch = true }: UseComprobanteProps) => {
    ...
    const fetchComprobante = async (codigo?: string) => {
        ...
        try {
            console.log(`[useComprobante] - Obteniendo comprobante para código desde URL: ${codigoToUse}`);
            ...
            if (response.success && response.data) {
                ...
                console.log(`[useComprobante] - Comprobante obtenido exitosamente:`, {
                    ...
                    clienteNombre: `${response.data.pago?.nombres_persona_paga} ${response.data.pago?.apellidos_persona_paga}`,
                    clienteEmail: response.data.pago?.email_persona_paga
                });
            } else {
                setError(response.detail || 'No se pudo obtener el comprobante');
                console.warn(`[useComprobante] - Error al obtener comprobante:`, response.detail);
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (autoFetch && token && codPagoRealizado && codPagoRealizado.trim() !== '') {
            console.log(`[useComprobante] - Auto-fetch activado para código desde URL: ${codPagoRealizado}`);
            fetchComprobante();
        }
    ...
    const refetch = () => {
        console.log(`[useComprobante] - Refetch manual solicitado para código: ${codPagoRealizado}`);
        fetchComprobante();
    };

    // Función para limpiar los datos
    const clearComprobante = () => {
        console.log(`[useComprobante] - Limpiando datos del comprobante`);
        setComprobante(null);
        setError(null);
    };
    ...
    const fetchByCode = (codigo: string) => {
        console.log(`[useComprobante] - Fetch manual con código específico: ${codigo}`);
