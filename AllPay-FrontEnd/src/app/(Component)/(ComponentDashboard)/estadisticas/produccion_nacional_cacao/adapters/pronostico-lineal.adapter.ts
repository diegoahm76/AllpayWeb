...
export async function fetchPronosticoLineal(
    token: string, 
    params: ParamsPronosticoLineal
): Promise<PronosticoLinealResponse> {
    try {
        const response = await axios.get(
            `${baseApiUrl}reportes/api/tablero11/pronostico-lineal/`, 
            {
                headers: {
        ...
        if (response.data.success === false) {
            throw new Error(response.data.detail || 'Error al obtener el pronóstico lineal');
        }
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (error.response?.status === 404) {
                throw new Error('No se encontró información de pronóstico lineal para los parámetros especificados');
            }
            
            if (error.response?.status === 500) {
                throw new Error('Error interno del servidor al obtener el pronóstico lineal');
            }
        }
        
        throw new Error(error.response?.data?.detail || error.message || 'No se pudo obtener el pronóstico lineal');
    }
}
