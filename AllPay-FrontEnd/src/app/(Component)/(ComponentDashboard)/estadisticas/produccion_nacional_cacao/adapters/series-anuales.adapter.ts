...
export async function fetchSeriesAnuales(
  token: string, 
  params: ParamsSeriesAnuales
): Promise<SeriesAnualesResponse> {
  try {
    const response = await axios.get(
      `${baseApiUrl}reportes/api/tablero11/series-anuales/`, 
      {
        headers: {
    ...
    if (response.data.success === false) {
      throw new Error(response.data.detail || 'Error al obtener series anuales de producción');
    }
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 404) {
        throw new Error('No se encontraron las series anuales de producción');
      }
    ...
    throw new Error('No se pudieron obtener las series anuales de producción');
  }
}
