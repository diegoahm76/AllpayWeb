...
export async function fetchSerieNalNy(
  token: string, 
  params: ParamsSerieNalNy
): Promise<SerieNalNyResponse> {
  try {
    const response = await axios.get(
      `${baseApiUrl}reportes/tablero6/serie-nal-ny/`, 
      {
        headers: {
    ...
    if (response.data.success === false) {
      throw new Error(response.data.message || 'Error al obtener serie NAL vs N.Y.');
    }
    ...
    throw new Error('No se pudo obtener la serie NAL vs N.Y.');
  }
}
