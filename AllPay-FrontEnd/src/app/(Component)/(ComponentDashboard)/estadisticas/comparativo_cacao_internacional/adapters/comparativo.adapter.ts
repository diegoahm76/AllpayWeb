...
export async function fetchComparativoInternacional(
  token: string, 
  params: ParamsComparativo
): Promise<ComparativoInternacionalResponse> {
  try {
    const response = await axios.get(
      `${baseApiUrl}reportes/tablero6/promedios-mensuales-comparado/`, 
      {
        headers: {
        ...
        params: {
          ...
          fecha_inicio1: params.fecha_inicio1,
          fecha_fin1: params.fecha_fin1,
          fecha_inicio2: params.fecha_inicio2,
          fecha_fin2: params.fecha_fin2
        }
      }
    );

    if (response.data.success === false) {
      throw new Error(response.data.message || 'Error al obtener comparativo internacional');
    }
    ...
    throw new Error('No se pudo obtener el comparativo internacional');
  }
}
